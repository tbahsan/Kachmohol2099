...
export async function listRecovery(projectId:string){const items=await db.snapshots.where('projectId').equals(projectId).sortBy('createdAt');return items.reverse()}

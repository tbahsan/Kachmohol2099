...
import { Activity, BookOpen, Box, Camera, ChevronRight, CircleDot, Copy, Download, DownloadCloud, FlipHorizontal, Folder, FolderOpen, Grid3X3, Headphones, Heart, Home, Image, Languages, Layers, LayoutDashboard, LockKeyhole, Maximize2, Minimize2, Moon, Move3D, Pause, Play, Plus, Redo2, RefreshCw, Rotate3D, Scale3D, Share2, Sparkles, Sun, Timer, Trash2, Trophy, Undo2, Upload, UserRound, WandSparkles, WifiOff, Wind, X, ZoomIn, ZoomOut, ScanSearch } from 'lucide-react'
...
type IdentityStyle='orbit'|'glass'|'gold'|'minimal'
type ExplorerIdentity={name:string;style:IdentityStyle;color:string;visible:boolean;includeInExport:boolean}
const loadIdentity=():ExplorerIdentity=>{try{return{ name:'',style:'orbit',color:'#62eff7',visible:true,includeInExport:false,...JSON.parse(localStorage.getItem('kachmohol-identity')||'{}')}}catch{return{name:'',style:'orbit',color:'#62eff7',visible:true,includeInExport:false}}}
...
function App() {
  ...
  const [identity, setIdentity] = useState<ExplorerIdentity>(loadIdentity)
  const [zoomCommand, setZoomCommand] = useState('reset:0')
  ...
  useEffect(() => {
    ...
    const focus=()=>setZoomCommand(`focus:${Date.now()}`)
    window.addEventListener('kachmohol-open-inspector', open)
    window.addEventListener('kachmohol-focus-object', focus)
    return () => {window.removeEventListener('kachmohol-open-inspector', open);window.removeEventListener('kachmohol-focus-object',focus)}
  }, [])
  useEffect(() => {
  ...
  useEffect(()=>{localStorage.setItem('kachmohol-identity',JSON.stringify(identity))},[identity])
  ...
  const saveBlob=(blob:Blob,filename:string)=>{const url=URL.createObjectURL(blob);const link=document.createElement('a');link.href=url;link.download=filename;link.style.display='none';document.body.appendChild(link);link.click();window.setTimeout(()=>{link.remove();URL.revokeObjectURL(url)},1000)}
  const waitForFrame=()=>new Promise<void>(resolve=>requestAnimationFrame(()=>requestAnimationFrame(()=>resolve())))
  const captureWallpaper=async(width?:number,height?:number,label='viewport')=>{try{setCapturing(true);await waitForFrame();const source=document.querySelector('#terrarium-canvas') as HTMLCanvasElement|null;if(!source)throw new Error('Canvas unavailable');if(!width||!height){const blob=await new Promise<Blob|null>(resolve=>source.toBlob(resolve,'image/png'));if(!blob)throw new Error('Capture failed');saveBlob(blob,`kachmohol-${label}.png`)}else{const memory=(navigator as Navigator&{deviceMemory?:number}).deviceMemory??4;const safePixels=width*height>7_000_000&&memory<6?{width:2560,height:1440}:{width,height};const output=document.createElement('canvas');output.width=safePixels.width;output.height=safePixels.height;const context=output.getContext('2d',{alpha:false});if(!context)throw new Error('Canvas unavailable');const scale=Math.max(safePixels.width/source.width,safePixels.height/source.height);const drawWidth=source.width*scale,drawHeight=source.height*scale;context.fillStyle='#02050c';context.fillRect(0,0,safePixels.width,safePixels.height);context.drawImage(source,(safePixels.width-drawWidth)/2,(safePixels.height-drawHeight)/2,drawWidth,drawHeight);const blob=await new Promise<Blob|null>(resolve=>output.toBlob(resolve,'image/png'));if(!blob)throw new Error('Capture failed');saveBlob(blob,`kachmohol-${label}.png`)}setNotice(store.language==='bn'?'ছবি download হয়েছে':'Image downloaded')}catch(error){console.error(error);setNotice(store.language==='bn'?'Screenshot তৈরি করা যায়নি':'Screenshot could not be created')}finally{setCapturing(false)}}
  const takePhoto=()=>captureWallpaper(undefined,undefined,'viewport')
  const downloadWallpaper=(width:number,height:number,label:string)=>captureWallpaper(width,height,label)
  ...
  const exportProfile=()=>{const profile={version:1,exportedAt:new Date().toISOString(),language:store.language,achievements:JSON.parse(localStorage.getItem('kachmohol-achievements')||'[]'),focusSessions:Number(localStorage.getItem('kachmohol-focus-sessions')||0),audioLevels,reducedMotion,identity};const blob=new Blob([JSON.stringify(profile,null,2)],{type:'application/json'});const link=document.createElement('a');link.href=URL.createObjectURL(blob);link.download='kachmohol-profile.json';link.click();URL.revokeObjectURL(link.href)}
  const importProfile=async(file?:File)=>{if(!file)return;try{const profile=JSON.parse(await file.text());if(!Array.isArray(profile.achievements))throw new Error();localStorage.setItem('kachmohol-achievements',JSON.stringify(profile.achievements));localStorage.setItem('kachmohol-focus-sessions',String(Number(profile.focusSessions)||0));if(profile.language==='bn'||profile.language==='en')store.setLanguage(profile.language);if(profile.audioLevels)setAudioLevels(profile.audioLevels);if(profile.identity)setIdentity(value=>({...value,...profile.identity}));setReducedMotion(Boolean(profile.reducedMotion));setNotice(store.language==='bn'?'Profile restore হয়েছে':'Profile restored')}catch{setNotice(store.language==='bn'?'Profile file সঠিক নয়':'Invalid profile file')}}
  ...
  const beginProject = (template: 'empty' | 'garden' | 'abyss') => { store.newProject(template); setHomeOpen(false); setPendingType(null); setInspectorOpen(false) }
  ...
  return <main className={`app-shell ${relaxMode ? 'relax-mode' : ''} ${capturing?'capturing':''}`}>
    <header className="topbar">
      ...
      <div className="top-actions">
        ...
        <IconButton title={identity.name||'Explorer identity'} onClick={()=>setProfileOpen(true)}><UserRound /></IconButton>
    ...
    <section className={`workspace ${advanced ? 'advanced' : 'guided'} ${inspectorOpen ? 'show-inspector' : ''} ${catalogOpen ? 'catalog-open' : ''} ${pendingType ? 'placing' : ''}`}>
      ...
      <section className="viewport"><TerrariumCanvas pendingType={pendingType} advanced={advanced} relax={relaxMode} stability={ecosystem.stability} cameraPreset={cameraPreset} cinematic={cinematic} reducedMotion={reducedMotion} zoomCommand={zoomCommand} identity={{...identity,visible:identity.visible&&(!capturing||identity.includeInExport)}} onPlace={position => { if (!pendingType) return; store.addEntity(pendingType, position); setPendingType(null); setNotice(store.language === 'bn' ? 'আর্টিফ্যাক্টটি স্থাপন হয়েছে' : 'Artifact placed') }} />
        ...
        <div className="camera-zoom"><button title="Zoom out" onClick={()=>setZoomCommand(`out:${Date.now()}`)}><ZoomOut/></button><button title="Reset view" onClick={()=>setZoomCommand(`reset:${Date.now()}`)}>1:1</button><button title="Zoom in" onClick={()=>setZoomCommand(`in:${Date.now()}`)}><ZoomIn/></button><button title="Focus selected" disabled={!selected} onClick={()=>setZoomCommand(`focus:${Date.now()}`)}><ScanSearch/></button></div>
    ...
    {relaxMode && <div className="relax-overlay"><div><strong>{identity.name?(store.language==='bn'?`${identity.name}-এর Sanctuary`:`Welcome back, ${identity.name}.`):primaryName}</strong><small>{store.environment.mode === 'day' ? t.day : t.night} · {store.entities.length} artifacts</small></div><a href="https://github.com/tbahsan" target="_blank" rel="noreferrer">by tbahsan</a><span className="relax-controls"><button className={cinematic?'active':''} onClick={()=>setCinematic(value=>!value)}><Camera/>{store.language==='bn'?'সিনেমাটিক':'Cinematic'}</button><button onClick={exitRelax}><Minimize2 />{store.language === 'bn' ? 'বের হন' : 'Exit'}</button></span></div>}
    ...
    {zenOpen && <div className="dashboard-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)setZenOpen(false)}}><section className="zen-studio">
      ...
      <section><h3><Image/>{store.language==='bn'?'ওয়ালপেপার':'Wallpaper studio'}</h3><div className="wallpaper-grid"><button onClick={()=>downloadWallpaper(1920,1080,'desktop-hd')}>Desktop<small>1920 × 1080</small></button><button onClick={()=>downloadWallpaper(1440,2560,'mobile')}>Mobile<small>1440 × 2560</small></button><button onClick={()=>downloadWallpaper(2560,1440,'desktop-2k')}>Desktop 2K<small>2560 × 1440</small></button><button onClick={()=>downloadWallpaper(3840,2160,'desktop-4k')}>Desktop 4K<small>Capability-safe fallback</small></button><button onClick={()=>downloadWallpaper(2048,2048,'square')}>Square<small>2048 × 2048</small></button><button onClick={takePhoto}>Viewport<small>Current size</small></button></div></section></div>
    </section></div>}
    ...
    {breathing && <div className="breathing-overlay"><button onClick={()=>setBreathing(false)}><X/></button><div className={`breath-orb ${breathPhase}`}><span>{identity.name?`${identity.name}, `:''}{breathPhase==='inhale'?(store.language==='bn'?'শ্বাস নিন':'breathe in'):breathPhase==='hold'?(store.language==='bn'?'ধরে রাখুন':'hold'):(store.language==='bn'?'শ্বাস ছাড়ুন':'breathe out')}</span></div><small>4 · 4 · 6</small></div>}
    {profileOpen&&<div className="dashboard-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)setProfileOpen(false)}}><section className="identity-studio"><header><div><small>PERSONAL SANCTUARY</small><h2>{store.language==='bn'?'Explorer পরিচয়':'Explorer Identity'}</h2></div><button onClick={()=>setProfileOpen(false)}><X/></button></header><div className="identity-preview" style={{'--identity-color':identity.style==='gold'?'#ffd977':identity.color} as React.CSSProperties}><UserRound/><small>SANCTUARY · 2099</small><strong>{identity.name||'EXPLORER'}</strong></div><label>{store.language==='bn'?'নাম':'Display name'}<input maxLength={28} value={identity.name} onChange={event=>setIdentity(value=>({...value,name:event.target.value}))}/></label><div className="identity-styles">{(['orbit','glass','gold','minimal'] as IdentityStyle[]).map(style=><button className={identity.style===style?'active':''} onClick={()=>setIdentity(value=>({...value,style}))} key={style}>{style}</button>)}</div><label className="identity-color">{store.language==='bn'?'আভার রং':'Glow color'}<input type="color" value={identity.color} onChange={event=>setIdentity(value=>({...value,color:event.target.value}))}/></label><label className="toggle wide"><span>{store.language==='bn'?'ডোমে name plaque দেখান':'Show name plaque in the dome'}</span><input type="checkbox" checked={identity.visible} onChange={event=>setIdentity(value=>({...value,visible:event.target.checked}))}/><i/></label><label className="toggle wide"><span>{store.language==='bn'?'Export-এ creator name রাখুন':'Include creator name in exports'}</span><input type="checkbox" checked={identity.includeInExport} onChange={event=>setIdentity(value=>({...value,includeInExport:event.target.checked}))}/><i/></label><button className="identity-save" onClick={()=>setProfileOpen(false)}>{store.language==='bn'?'Sanctuary সংরক্ষণ':'Save sanctuary identity'}</button></section></div>}
    {homeOpen && <div className="home-hub">
      <div className="home-glow" /><div className="home-content">
        <div className="portal-aurora a"/><div className="portal-aurora b"/><div className="portal-stars"/>
        <div className="home-brand"><div className="orb"><Sparkles /></div><small>{alternateName}</small><h1>{identity.name?(store.language==='bn'?`${identity.name}-এর কাচমহল`:`${identity.name}'s Kachmohol`):primaryName}</h1><p>{store.language === 'bn' ? 'আপনার নিজস্ব জীবন্ত কাচের জগৎ গড়ে তুলুন' : 'Create your own living world of glass and light'}</p></div>
        <label className="portal-identity"><UserRound/><span><small>{store.language==='bn'?'আপনাকে কী নামে ডাকব? · ঐচ্ছিক':'What should the sanctuary call you? · Optional'}</small><input maxLength={28} value={identity.name} placeholder={store.language==='bn'?'আপনার নাম':'Explorer name'} onChange={event=>setIdentity(value=>({...value,name:event.target.value}))}/></span></label>
